...
export default function FluxoConsolidadoSection({ data, regime, onRegime, onDrill }) {
  ...
  return (
    <section className="space-y-3 rounded-2xl border bg-card p-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-5"><KPI label="Faturamento válido" value={data.faturamento} sub="NFs, CIs e integrações válidas" icon={TrendingUp} onClick={() => onDrill('faturamentoConsolidado')} /><KPI label="Custos diretos" value={data.custosDiretos} sub={pct(data.faturamento ? data.custosDiretos / data.faturamento * 100 : 0)} icon={Wallet} /><KPI label="Margem direta" value={data.margemDireta} sub={pct(data.faturamento ? data.margemDireta / data.faturamento * 100 : 0)} icon={Scale} /><KPI label="Outras saídas" value={data.outrasSaidas} sub={pct(data.faturamento ? data.outrasSaidas / data.faturamento * 100 : 0)} icon={ArrowDownCircle} /><KPI label="Resultado final" value={data.resultado} sub={`${data.margemFinal.toFixed(1)}% de margem`} icon={CircleDollarSign} /></div>
    </section>
  );
}