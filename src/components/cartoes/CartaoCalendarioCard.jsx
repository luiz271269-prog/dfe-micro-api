...
export default function CartaoCalendarioCard({ cartao, latestFat, fileUrl, faturasCount, isExpanded, onToggle, onViewFile }) {
  ...
  return (
    <button
      onClick={onToggle}
      className={`group relative w-full text-left rounded-xl overflow-hidden bg-gradient-to-br px-1 ${b.gradient} shadow-md hover:shadow-lg transition-all ${isExpanded ? 'ring-2 ring-primary ring-offset-2' : ''}`}>
